...
class SmartComputerPlayer(ComputerPlayer):
    ...
    def __init__(self):
        ...
        self.previouslyUnknown = False

    def takeTurn(self, otherPlayer):
        r = 0
        c = 0
        # self.hypotheticalShips.printGrid()
        if self.firstShipHit[0] != -1 or self.firstShipHit[
            1] != -1:  # runs lateral movement code if it is targeting a ship
            ...
            if not self.previousHit:  # returns to initial hit when it passes the eddge of the ship
                ...
                c = self.lastShot[1]
            try:
                cannotMoveUp = not self.gridShots.isSpaceWater(r - 1,
                                                               c)  # checks if the space above has not been checked yet
            except IndexError:  # moving up is prevented by default if the index does not exist
                pass
            try:
                cannotMoveRight = not self.gridShots.isSpaceWater(r,
                                                                  c + 1)  # checks if the space to the right has not been checked yet
            except IndexError:  # moving right is prevented by default if the index does not exist
                pass
            try:
                cannotMoveDown = not self.gridShots.isSpaceWater(r + 1,
                                                                 c)  # checks if the space below has not been checked yet
            except IndexError:  # moving down is prevented by default if the index does not exist
                pass
            try:
                cannotMoveLeft = not self.gridShots.isSpaceWater(r,
                                                                 c - 1)  # checks if the space to the left has not been checked yet
        ...
        if self.previouslyUnknown:  # ensures that it does not get stuck on the same unknown spaces every time
            for x in range(r, 10):  # traverses the 2D grid in a row-by-row top-to-bottom left-to-right approach
                if x > r:  # resets leftmost checked spot after passing the last unknown space
                    c = 0
                for y in range(c, 10):
                    if self.hypotheticalShips.returnLocation(x, y) == "U" and [x,
                                                                               y] != self.firstShipHit:  # finds any unknown but not destroyed ships
                        # print("unknown ship at ", x, y)
                        self.firstShipHit = [x, y]  # begins targeting the unknown ship
                        self.lastShot = [x, y]
                        self.previousHit = True
                        self.takeTurn(otherPlayer)  # recalls method with new target
                        return  # already shot the ship by this point, so it exits to prevent a double shot
            self.firstShipHit = [-1, -1]  # resets targeting system
        else:
            for x in range(0, 10):  # traverses the 2D grid in a row-by-row top-to-bottom left-to-right approach
                for y in range(0, 10):
                    if self.hypotheticalShips.returnLocation(x, y) == "U" and [x,
                                                                               y] != self.firstShipHit:  # finds any unknown but not destroyed ships
                        # print("unknown ship at ", x, y)
    ...
    def shoot(self, otherPlayer, r, c):
        ...
        if otherPlayer.gridShips.isSpaceWater(r, c):  # the shot is a miss
            ...
            self.hypotheticalShips.changeSingleSpace(r, c, "0")
            print("Miss")
            self.previousHit = False
            ...
            isDestroyed = True
            for x in range(10):  # traverses 2D grid
                for y in range(10):
                    if otherPlayer.gridShips.returnLocation(x,
                                                            y) is destroyedShip:  # checks each space if any parts of the ship aren't sunk
                        isDestroyed = False
                        break
                if not isDestroyed:
                    break
            if isDestroyed:
                ...
                if self.directionOfNextShot == 0:  # maps the upward-facing ship on a more specific gridShots that indicates every sunk and unsunk point
                    # print(r, c)
                    self.hypotheticalShips.changeCol(c, destroyedShip, r, self.shipSize[destroyedShip])
                if self.directionOfNextShot == 1:  # maps the rightward-facing ship on a more specific gridShots that indicates every sunk and unsunk point
                    # print(r, c - self.shipSize[destroyedShip]+1)
                    self.hypotheticalShips.changeRow(r, destroyedShip, c - self.shipSize[destroyedShip] + 1,
                                                     self.shipSize[destroyedShip])
                if self.directionOfNextShot == 2:  # maps the downward-facing ship on a more specific gridShots that indicates every sunk and unsunk point
                    # print(r - self.shipSize[destroyedShip]+1, c)
                    self.hypotheticalShips.changeCol(c, destroyedShip, r - self.shipSize[destroyedShip] + 1,
                                                     self.shipSize[destroyedShip])
                if self.directionOfNextShot == 3:  # maps the leftward-facing ship on a more specific gridShots that indicates every sunk and unsunk point
                    # print(r, c)
                    self.hypotheticalShips.changeRow(r, destroyedShip, c, self.shipSize[destroyedShip])
                # self.hypotheticalShips.printGrid()
                self.firstShipHit = [-1, -1]
                self.previousHit = False
            else:
                print("Hit!")
                self.hypotheticalShips.changeSingleSpace(r, c, "U")
                self.previousHit = True
                # print(r, c)
