...
class ComputerPlayer(Player):
        ...
        while True:
            ...
            if self.gridShots.isSpaceWater(r, c):  # repeats previous step if it has already shot in that space
                if otherPlayer.gridShips.isSpaceWater(r, c):  # the shot is a miss
                    ...
                    destroyedShip = otherPlayer.gridShips.returnLocation(r, c).clone()  # store ship type for later
    ...
    def stillHasShips(self):
        for r in range(10):  # nested for loops traverse 2D array
            for c in range(10):
                if self.gridShips.returnLocation(r, c) is not "X" or self.gridShips.returnLocation(r, c) is not "0" or not self.gridShips.isSpaceWater(
                        r, c):
                    return False  # something other than a shot, miss, or water was detected which must be a ship
